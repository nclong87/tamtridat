...
class DataProvider
{
	...
	function updateCache() {
		mysql_query("update raovats set `status`=-2 where `status`!=-2 and expiredate is not null and expiredate < now()",$this->link) or die("Error:".mysql_error());
		mysql_query("update raovats set `status`=1 where `status`=-2 and expiredate is not null and expiredate > now()",$this->link) or die("Error:".mysql_error());
